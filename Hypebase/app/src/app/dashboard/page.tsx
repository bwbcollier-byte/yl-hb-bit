...
import { 
  ...
  Trash2, Edit2, Search, LayoutGrid, LayoutList, List, Plus, 
  ChevronDown, ArrowUpRight, ArrowDownRight, Building2, X, AlertCircle, ChevronUp
} from "lucide-react"
         ...
         <div className="flex-1 overflow-y-auto no-scrollbar p-8">
            <div className="max-w-[1400px] mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                  <div>
                    <h2 className="font-heading text-3xl font-extrabold tracking-tight mb-2">{greeting.text}, {userName} {greeting.emoji}</h2>
                    <p className="text-[var(--color-brand-muted)]">Welcome back to Hypebase</p>
                  </div>
                      ...
                      {createMenuOpen && (
                        <>
                          ...
                          <motion.div 
                            ...
                            className="absolute right-0 mt-2 w-56 bg-[var(--color-brand-surface-1)] border border-white/10 rounded-xl shadow-2xl py-2 z-20 origin-top-right overflow-hidden"
                          >
                            <CreateMenuItem href="/dashboard/talent/new" label="Talent Profile" icon={Users2} onClick={() => setCreateMenuOpen(false)} />
                            <CreateMenuItem href="/dashboard/companies/new" label="Company" icon={Building2} onClick={() => setCreateMenuOpen(false)} />
                            <CreateMenuItem href="/dashboard/contacts/new" label="Contact" icon={BadgeDollarSign} onClick={() => setCreateMenuOpen(false)} />
                            <CreateMenuItem href="/dashboard/events/new" label="Event" icon={CheckCircle2} onClick={() => setCreateMenuOpen(false)} />
                            <CreateMenuItem href="/dashboard/clubs/new" label="Clubs & Groups" icon={Users2} onClick={() => setCreateMenuOpen(false)} />
                          </motion.div>
                        </>
                      )}
                    </AnimatePresence>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                  ...
                  <div className="lg:col-span-1 bg-[#09090b] shadow-[0_2px_8px_rgba(0,0,0,0.5),0_0_0_1px_rgba(255,255,255,0.05)] border border-white/5 rounded-lg p-5 flex flex-col lg:row-span-2 h-full">
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        <h3 className="font-bold text-lg">Deals signed</h3>
                  ...
                  <div className="lg:col-span-3 bg-[#09090b] shadow-[0_2px_8px_rgba(0,0,0,0.5),0_0_0_1px_rgba(255,255,255,0.05)] border border-white/5 rounded-lg p-5 flex flex-col">
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        <h3 className="font-bold text-lg">Application users</h3>
...
function CreateMenuItem({ href, label, icon: Icon, onClick }: { href: string, label: string, icon: any, onClick: () => void }) {
  return (
    <Link href={href} onClick={onClick}>
      <div className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition-colors cursor-pointer group">
        <div className="w-8 h-8 rounded-lg bg-white/5 border border-white/5 flex items-center justify-center text-[var(--color-brand-muted)] group-hover:text-[var(--color-brand-violet)] transition-colors">
          <Icon size={16} />
        </div>
