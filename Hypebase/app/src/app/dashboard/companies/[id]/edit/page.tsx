          ...
          <div className="max-w-4xl mx-auto pb-20">
            ...
            <div className="bg-[var(--color-brand-surface-1)] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 bg-gradient-to-r from-[var(--color-brand-surface-2)]/50 to-transparent flex items-center justify-between">
                <div>
                  <h1 className="text-2xl font-bold font-heading mb-1">Edit Company</h1>
                  <p className="text-[var(--color-brand-muted)] text-sm">Update company identity, location, and corporate details.</p>
                </div>
                {companyData?.logo && (
                   <div className="w-16 h-16 rounded-xl border border-white/10 p-2 overflow-hidden shadow-xl bg-white/5">
                      <img src={companyData.logo} alt={companyData.name} className="w-full h-full object-contain" />
                   </div>
                )}
              </div>
              ...
              <div className="p-8">
                {error ? (
                  ...
                  <CompanyForm initialData={companyData} id={id as string} />
                )}
              </div>
            </div>
          </div>
