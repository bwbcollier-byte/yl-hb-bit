...
export default function EditProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        const { data: projectData, error: err } = await supabase
          .from("projects")
          .select("*")
          ...
          <div className="max-w-4xl mx-auto pb-20">
            ...
            <div className="bg-[var(--color-brand-surface-1)] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 bg-gradient-to-r from-[var(--color-brand-surface-2)]/50 to-transparent">
                <h1 className="text-2xl font-bold font-heading mb-1">Edit Project</h1>
                <p className="text-[var(--color-brand-muted)] text-sm">Update the scope and details of this endeavor.</p>
              </div>
              ...
              <div className="p-8">
                {loading ? (
                  ...
                  <ProjectForm initialData={data} id={resolvedParams.id} />
                ) : (
