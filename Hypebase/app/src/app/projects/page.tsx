...
import { 
  Clapperboard, Plus, Filter, Search, MapPin, 
  ChevronDown, ArrowDown, ArrowUp, LayoutGrid, 
  LayoutList, List, Edit2, Briefcase, CheckCircle2, AlertCircle,
         ...
         <div className="flex-1 overflow-y-auto no-scrollbar p-8 pb-32">
            <div className="max-w-[1400px] mx-auto">
                <div className="px-2 mb-10">
                  <h2 className="font-heading text-3xl font-extrabold tracking-tight mb-2 text-[var(--color-brand-text)]">Projects & Productions</h2>
                  <p className="text-[var(--color-brand-muted)] mb-8">Manage active films, TV shows, endorsements, and creative endeavors.</p>

                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    {[
                      { label: 'Total Projects', value: totalCount.toString(), icon: Clapperboard, growth: '+2' },
                      { label: 'In Production', value: '12', icon: Briefcase, growth: 'Steady' },
                      { label: 'Completed', value: '84', icon: CheckCircle2, growth: '+4' },
                      { label: 'Pending Start', value: '3', icon: Clock, growth: '-1' }
                    ].map((card, i) => (
                        ...
                        <div className="relative" ref={sortRef}>
                          ...
                          {sortOpen && (
                            <div className="absolute top-12 left-0 w-44 bg-[var(--color-brand-surface-1)] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden text-sm font-semibold animate-in fade-in zoom-in-95 duration-200">
                              {[
                                { label: "Project Name", val: "name" },
                                { label: "Creation Date", val: "created_at" },
                ...
                <div className={
                  viewType === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" :
                  viewType === 'medium' ? "grid grid-cols-1 md:grid-cols-2 gap-6" :
                  "space-y-4"
                }>
                  {isLoading ? (
                    Array.from({ length: itemsPerPage }).map((_, i) => (
                      <div key={i} className={`animate-pulse bg-white/5 p-4 rounded-xl border border-white/5 ${viewType === 'list' ? 'flex items-center gap-4' : ''}`}>
                         <div className={`bg-white/10 rounded-lg ${viewType === 'grid' ? 'h-32 w-full mb-4' : 'w-16 h-12 shrink-0'}`} />
                    ...
                    projects.map((project) => (
                      viewType === 'grid' ? (
                        <div key={project.id} onClick={(e) => navigateToEdit(project.id, e)} className="group flex flex-col h-full cursor-pointer relative text-left bg-[var(--color-brand-surface-1)] p-5 rounded-2xl border border-white/5 shadow-lg hover:border-[var(--color-brand-violet)]/40 transition-all">
                          <div className="flex items-start justify-between mb-4">
                             <div className="w-12 h-12 bg-black/40 rounded-xl flex items-center justify-center border border-white/10 group-hover:border-[var(--color-brand-violet)]/30 overflow-hidden shrink-0 shadow-inner">
                                <Clapperboard size={20} className="text-[var(--color-brand-muted)] group-hover:text-[var(--color-brand-violet)] transition-colors" />
                             </div>
                             <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--color-brand-muted)] bg-white/5 px-2 py-1 rounded-md">
                                {project.project_type || 'Project'}
                             </span>
                          </div>
                          
                          <h4 className="font-heading text-lg font-bold text-white tracking-tight line-clamp-2 leading-tight mb-2 group-hover:text-[var(--color-brand-violet)] transition-colors">{project.name}</h4>
                          
                          <p className="text-[11px] font-medium text-[var(--color-brand-muted)] tracking-wider flex items-center gap-1.5 mb-4">
                            <MapPin size={12} className="text-[var(--color-brand-violet)]" /> {project.location || 'Location Unknown'}
                          </p>
                          
                          <p className="text-xs text-[var(--color-brand-muted)]/80 line-clamp-3 leading-relaxed mt-auto border-t border-white/5 pt-4">
                             {project.description || 'No description provided.'}
                          </p>
                        </div>
                      ) : (
                        // Simplified row/card for other views
                        <div key={project.id} onClick={(e) => navigateToEdit(project.id, e)} className="bg-[var(--color-brand-surface-1)] border border-white/5 p-4 rounded-2xl flex items-center gap-5 hover:border-[var(--color-brand-violet)]/30 transition-all cursor-pointer group shadow-lg">
                           <div className="w-14 h-14 bg-black/40 rounded-xl flex items-center justify-center border border-white/10 group-hover:border-[var(--color-brand-violet)]/30 shrink-0 shadow-inner">
                              <Clapperboard size={24} className="text-[var(--color-brand-muted)] group-hover:text-[var(--color-brand-violet)] transition-colors" />
                           </div>
                           <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-3 mb-1">
                                <h4 className="font-bold text-white text-base truncate group-hover:text-[var(--color-brand-violet)] transition-colors">
                                    {project.name} 
                                </h4>
                                <span className="text-[10px] font-bold text-[var(--color-brand-muted)] uppercase px-2 py-0.5 rounded bg-white/5 whitespace-nowrap">{project.project_type}</span>
                              </div>
                              <div className="flex items-center gap-4 text-[11px] text-[var(--color-brand-muted)] font-medium uppercase tracking-wider">
                                 <span className="flex items-center gap-1.5"><MapPin size={12} className="text-[var(--color-brand-violet)]" /> {project.location || 'TBA'}</span>
                              </div>
                           </div>
                           <div className="flex items-center gap-2">
                              <button className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 transition-colors opacity-0 group-hover:opacity-100 flex items-center justify-center">
                                <Edit2 size={16} />
                              </button>
                           </div>
                        </div>
                    ...
                    <div className="col-span-full py-24 flex flex-col items-center justify-center text-center">
                       <div className="w-20 h-20 rounded-3xl bg-[var(--color-brand-surface-2)] flex items-center justify-center mb-6 shadow-2xl border border-white/5">
                          <AlertCircle size={32} className="text-[var(--color-brand-muted)]" />
                       </div>
                       <h3 className="text-2xl font-heading font-extrabold text-white mb-2 tracking-tight">No projects listed</h3>
                       <p className="text-[var(--color-brand-muted)] max-w-sm mb-8 font-medium">Create your first production or endorsement project.</p>
                       <Link href="/dashboard/projects/new">
                        <button className="border border-[var(--color-brand-violet)] bg-[#09090b] hover:bg-[var(--color-brand-violet)]/10 text-[var(--color-brand-violet)] font-bold px-6 py-3 rounded-lg flex items-center justify-center gap-2 transition-colors shadow-[0_0_10px_rgba(138,43,226,0.15)]">Create First Project</button>
                       </Link>
                    </div>
                  )}
                </div>
