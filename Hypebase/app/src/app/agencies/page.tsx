...
import { 
  Briefcase, Plus, Filter, Search, MapPin, 
  ChevronDown, X, Heart, ArrowDown, ArrowUp, LayoutGrid, 
  LayoutList, List, ChevronLeft, ChevronRight, 
  ExternalLink, Edit2, CheckCircle2, Globe, Building2,
  MoreVertical, ChevronUp, Users, BadgeDollarSign, Mic, Camera, Trophy, Layers, TableProperties,
  Trash2, XCircle
} from "lucide-react"
import { useState, useRef, useEffect } from "react"
import { supabase } from "@/lib/supabase"
import { CompanyDrawer } from "@/components/directory-drawers"
...
export default function CompaniesDirectory() {
  ...
  const companyStatusOptions: DropdownOption[] = [
    ...
    { id: "client", label: "Client", icon: CheckCircle2, color: "#4ECDC4" },
    { id: "lead", label: "Lead", icon: Briefcase, color: "#F9C74F" }
  ]
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        if (debouncedSearch) {
          query = query.or(`name.ilike.%${debouncedSearch}%,type.ilike.%${debouncedSearch}%,description.ilike.%${debouncedSearch}%`)
        }
         ...
         <div className="flex-1 overflow-y-auto no-scrollbar p-8 pb-32">
            <div className="max-w-[1400px] mx-auto">
                <div className="px-2 mb-10">
                  <h2 className="font-heading text-3xl font-extrabold tracking-tight mb-2 text-[var(--color-brand-text)]">Companies Directory</h2>
                  <p className="text-[var(--color-brand-muted)] mb-8">Manage partnerships, agencies, and corporate entities in your ecosystem.</p>

                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    {[
                      { label: 'Total Companies', value: '1,240', icon: Building2, growth: '+8%' },
                      { label: 'Active Agencies', value: '312', icon: Briefcase, growth: '+12%' },
                      { label: 'Total Employees', value: '45.2k', icon: Users, growth: '+5%' },
                      { label: 'Annual Revenue', value: '$124M', icon: BadgeDollarSign, growth: '+20%' }
                    ].map((card, i) => (
                      <BudgetCard 
                        key={i}
                        title={card.label}
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex-1 max-w-md relative">
                      ...
                      <input 
                        ...
                        placeholder="Search companies, agencies, industries..." 
                    ...
                    <div className="flex flex-wrap items-center gap-3 relative" ref={filterRef}>
                          ...
                          <ActionsDropdown
                            ...
                            actions={[
                              { id: 'set-active', label: 'Set Active', icon: <CheckCircle2 size={14} className="text-[#00FA9A]" />, onClick: async (ids) => {
                                await Promise.all(ids.map(id => supabase.from('company_profiles').update({ status: 'active' }).eq('id', id)));
                                setCompanies(prev => prev.map(c => ids.includes(c.id) ? { ...c, status: 'active' } : c));
                              ...
                              { id: 'set-inactive', label: 'Set Inactive', icon: <XCircle size={14} className="text-orange-400" />, onClick: async (ids) => {
                                await Promise.all(ids.map(id => supabase.from('company_profiles').update({ status: 'inactive' }).eq('id', id)));
                                setCompanies(prev => prev.map(c => ids.includes(c.id) ? { ...c, status: 'inactive' } : c));
                                setSelectedEditorIds(new Set());
                              }},
                              { id: 'delete', label: 'Delete Selected', icon: <Trash2 size={14} />, variant: 'danger', onClick: async (ids) => {
                                await Promise.all(ids.map(id => supabase.from('company_profiles').delete().eq('id', id)));
                      ...
                      <div className="relative">
                        <button onClick={() => setFilterOpen(!filterOpen)} className={`w-10 h-10 rounded-lg border flex items-center justify-center bg-[var(--color-brand-surface-1)] text-[var(--color-brand-muted)] border-white/10 hover:text-white transition-all ${filterOpen ? 'bg-[var(--color-brand-violet)]/10 border-[var(--color-brand-violet)]/30 text-[var(--color-brand-violet)]' : ''}`}>
                          <Filter size={18} />
                        </button>
                        <AnimatePresence>
                          {filterOpen && (
                            ...
                            >
                               ...
                               <div className="space-y-4">
                                  <div className="space-y-2">
                                     <label className="text-xs font-bold text-white/70">Company Status</label>
                                     <FluidDropdown 
                                        options={companyStatusOptions}
                                        value={companyStatus}
                                        onChange={(val) => { setCompanyStatus(val); setPage(1); }}
                                     />
                                  </div>
                                  <div className="space-y-2">
                ...
                {editorMode ? (
                  <EditorTable
                    columns={[
                      ...
                      { key: 'title', label: 'Company', render: (val: any) => <span className="font-bold text-white text-[13px]">{val}</span> },
                      { key: 'subtitle', label: 'Type', render: (val: any) => <span className="text-white/50 text-[13px]">{val || '\u2014'}</span> },
                      { key: 'status', label: 'Status', width: '120px', render: (val: any, row: EditorRowData) => (
                        <div 
                          ...
                          onClick={(e) => {
                            ...
                            supabase.from('company_profiles').update({ status: newStatus }).eq('id', row.id).then(() => {
                              setCompanies(prev => prev.map(c => c.id === row.id ? { ...c, status: newStatus } : c));
                            });
                          }}
                        >
                    ...
                    onEdit={(id) => router.push(`/dashboard/companies/${id}/edit`)}
                    onDelete={async (id) => {
                ...
                <div className={
                  ...
                  {isLoading ? (
                    ...
                    companies.map((company) => (
                      viewType === 'grid' ? (
                        <div key={company.id} onClick={() => setSelectedCompany(company)} className="group flex flex-col h-full cursor-pointer relative text-center">
                          <div className="w-full aspect-square rounded-lg overflow-hidden mb-4 relative bg-[#1A1A1A] border border-white/5 shadow-2xl flex items-center justify-center group-hover:border-[var(--color-brand-violet)]/40 transition-all">
                             <img 
                                src={company.logo_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(company.name)}&background=random&size=400`} 
                                alt={company.name} 
                                className="w-full h-full object-contain p-8 transition-transform duration-700 group-hover:scale-110" 
                             />
                             <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                               <button onPointerDown={(e) => e.stopPropagation()} onClick={(e) => toggleLike(company.id, e)} className="p-2 rounded-lg bg-black/60 backdrop-blur-md border border-white/10 hover:bg-black/80 z-20 shadow-xl transition-all"><Heart size={16} className={likedIds.includes(company.id) ? "text-red-500 fill-red-500" : "text-white"} /></button>
                               <button onPointerDown={(e) => e.stopPropagation()} onClick={(e) => navigateToEdit(company.id, e)} className="p-2 rounded-lg bg-black/60 backdrop-blur-md border border-white/10 hover:bg-white/20 z-20 shadow-xl transition-all"><Edit2 size={16} /></button>
                             </div>
                          </div>
                          <h4 className="font-heading text-base font-bold text-white tracking-tight truncate border-b border-transparent group-hover:border-[var(--color-brand-violet)]/20 transition-all inline-block mx-auto pb-0.5">{company.name}</h4>
                          <p className="text-[11px] font-black text-[var(--color-brand-muted)] truncate mt-1 tracking-widest uppercase opacity-60 group-hover:opacity-100 transition-opacity">{company.type || 'Partnership'}</p>
                        </div>
                      ) : (
                        <div key={company.id} onClick={() => setSelectedCompany(company)} className="bg-[var(--color-brand-surface-1)] border border-white/5 p-4 rounded-xl flex items-center gap-4 hover:border-[var(--color-brand-violet)]/30 transition-all cursor-pointer group shadow-2xl hover:bg-white/5">
                           <div className="w-12 h-12 rounded-lg bg-white/5 border border-white/5 flex items-center justify-center p-2 shrink-0 shadow-sm">
                             <img src={company.logo_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(company.name)}&size=100`} alt={company.name} className="max-w-full max-h-full object-contain" />
                           </div>
                           <div className="flex-1 min-w-0">
                              <h4 className="font-bold text-white text-base truncate group-hover:text-[var(--color-brand-violet)] transition-colors">{company.name}</h4>
                              <div className="flex items-center gap-3 text-[10px] text-[var(--color-brand-muted)] font-black mt-1 uppercase tracking-widest">
                                 <span className="flex items-center gap-1.5"><Building2 size={10} className="text-[var(--color-brand-violet)]" /> {company.type}</span>
                                 <span className="flex items-center gap-1.5 border-l border-white/10 pl-3"><MapPin size={10} className="text-[var(--color-brand-violet)]" /> {company.location}</span>
                              </div>
                           </div>
                           <div className="flex items-center gap-2">
                              {company.website && <a href={company.website} target="_blank" rel="noreferrer" className="p-2.5 rounded-lg bg-white/5 border border-white/5 text-[var(--color-brand-muted)] hover:text-white hover:border-white/10 z-20 shadow-sm transition-all" onPointerDown={(e) => e.stopPropagation()} onClick={e => e.stopPropagation()}><Globe size={16} /></a>}
                              <button onPointerDown={(e) => e.stopPropagation()} onClick={(e) => navigateToEdit(company.id, e)} className="p-2.5 rounded-lg bg-white/5 border border-white/5 text-[var(--color-brand-muted)] hover:text-white hover:border-white/10 z-20 shadow-sm transition-all"><Edit2 size={16} /></button>
                           </div>
                        </div>
                      )
                    ))
                    ...
                    <div className="col-span-full py-24 flex flex-col items-center justify-center text-center">
                       <div className="w-20 h-20 rounded-lg bg-[var(--color-brand-surface-2)] flex items-center justify-center mb-6 shadow-2xl border border-white/5">
                          <Building2 size={32} className="text-[var(--color-brand-muted)]" />
                       </div>
                       <h3 className="text-2xl font-heading font-extrabold text-white mb-2 tracking-tight">No companies found</h3>
                       <p className="text-[var(--color-brand-muted)] max-w-sm mb-8 font-medium">Build your corporate relationships by adding agencies and brand partners.</p>
                       <Link href="/dashboard/companies/new"><button className="border border-[var(--color-brand-violet)] bg-[#09090b] hover:bg-[var(--color-brand-violet)]/10 text-[var(--color-brand-violet)] font-bold px-6 py-3 rounded-lg flex items-center justify-center gap-2 transition-colors shadow-[0_0_10px_rgba(138,43,226,0.15)]">Add First Company</button></Link>
                    </div>
                  )}
                </div>
                )}
