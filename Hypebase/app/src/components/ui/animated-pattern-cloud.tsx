...
export const ProceduralGroundBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        // Color Palette tailored to Hypebase
        vec3 baseColor = vec3(0.04, 0.04, 0.06); // Obsidian base
        vec3 accentColor = vec3(0.588, 0.0, 1.0); // Violet (#9600ff)
        vec3 neonColor = vec3(1.0, 0.0, 0.757);   // Pink (#ff00c1)
  ...
  return (
    <div className="absolute inset-0 w-full h-full -z-10 overflow-hidden rounded-b-[4rem]">
      <canvas
        ref={canvasRef}
        className="w-full h-full block touch-none"
        style={{ filter: 'contrast(1.1) brightness(0.9)' }}
      />
      {/* Fade out bottom edge to blend with next section */}
      <div className="absolute bottom-0 inset-x-0 h-32 bg-gradient-to-t from-[var(--color-brand-obsidian)] to-transparent" />
    </div>
  );
};
