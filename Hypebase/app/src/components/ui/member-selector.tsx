...
function AddButton({ onClick, isOpen }: AddButtonProps) {
  return (
    ...
    >
      <div
        className={cn(
          "w-9 h-9 rounded-full border-2 border-dashed flex items-center justify-center transition-all duration-200",
    ...
    return (
      <div ref={ref} className={cn("relative", className)}>
        ...
        >
          <LayoutGroup>
            <AnimatedTooltip 
              items={visibleMembers.map(m => ({ 
                id: m.id, 
                name: m.name, 
                designation: m.email || "Member", 
