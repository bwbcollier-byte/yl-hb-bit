  ...
  ({ image, title, delay, isVisible, index, totalCount, onClick, isSelected }, ref) => {
    ...
    return (
      <div
        ...
        style={{
          transform: isVisible
            ? `translateY(calc(-80px + ${translationY}px)) translateX(${translationX}px) rotate(${rotation}deg) scale(1)`
      ...
      >
        <div className={cn(
          ...
          <p className="absolute bottom-1.5 left-1.5 right-1.5 text-[10px] font-heading font-bold uppercase tracking-widest text-white truncate drop-shadow-md">
            {title}
          </p>
        </div>
      </div>
...
const AnimatedFolder: React.FC<AnimatedFolderProps> = ({ title, projects, className, gradient }) => {
  ...
  return (
    <>
      <div
        className={cn("relative flex flex-col items-center justify-center p-8 rounded-lg cursor-pointer bg-[var(--color-brand-surface-1)] border border-white/10 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] hover:shadow-2xl hover:border-white/20 hover:bg-white/5 group", className)}
        style={{ minWidth: "100%", minHeight: "340px", perspective: "1200px", transform: isHovered ? "scale(1.02)" : "scale(1)" }}
      ...
      >
        ...
        <p
          className="text-sm text-[var(--color-brand-muted)] transition-all duration-300"
          style={{
...
export const defaultPortfolioData = [
  ...
  {
    title: "USER PROFILES",
    gradient: "linear-gradient(135deg, #11998e, #38ef7d)", // Added some new gradients for the extra folders
    projects: [] as Project[]
  },
  {
    title: "TASK MANAGER",
    gradient: "linear-gradient(135deg, #108dc7, #ef8e38)",
    projects: [] as Project[]
  },
  {
...
export default function ThreeDFolders({ data = defaultPortfolioData }: { data?: PortfolioCategory[] }) {
  return (
    <div className="w-full flex object-top flex-col items-center">
      <section className="w-full max-w-7xl mx-auto px-6 pt-8 pb-32">
