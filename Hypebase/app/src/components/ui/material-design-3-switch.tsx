...
const MaterialSwitch = React.forwardRef<HTMLInputElement, SwitchProps>(
  ({ 
    className, 
    size, 
    variant, 
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              className={cn(
                "relative top-1/2 flex -translate-y-1/2 items-center justify-center rounded-full bg-background shadow-sm transition-all duration-300 ease-[var(--ease-spring)]",
                isChecked 
                  ? handleSizeChecked
                  : isPressed || isHovered
                    ? handleSizePressed
                    : handleSizeUnchecked,
                isHovered && !isChecked && !isPressed && "w-3.5 h-3.5 ml-[2px]", // Hover state adjustment (M3 spec: unselected handle becomes slightly larger)
              )}
            >
              {/* --- ICONS CONATINER --- */}
              <div
                className={cn(
                  "relative flex items-center justify-center h-full w-full transition-opacity duration-200",
                  shouldRenderIcons ? "opacity-100" : "opacity-0"
                )}
              >
                  {isChecked ? (
                     checkedIcon || (showIcons && <Check className={cn("text-primary", iconClasses)} strokeWidth={3} />)
                  ) : (
                     uncheckedIcon || (showIcons && <X className={cn("text-muted-foreground", iconClasses, isSmall ? "w-2 h-2" : "w-3 h-3")} strokeWidth={3} />)
                  )}
              </div>
            </div>
          </div>
        ...
        {/* --- STATE LAYER (Hover / Focus visualizer) --- */}
        <div 
          className={cn(
            "absolute left-[2px] top-1/2 h-10 w-10 -translate-y-1/2 rounded-full opacity-0 pointer-events-none transition-all duration-300",
            "bg-foreground/10",
            "group-focus-visible:opacity-100",
            isHovered && !isPressed && !disabled && "opacity-100 scale-110",
            isChecked ? "left-auto right-[2px] bg-primary/20" : ""
          )} 
        />
      </label>
    );
